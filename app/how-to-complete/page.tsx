...
export default function HowToCompleteMigrated() {
  useEffect(() => {
    const init = async () => {
      try {
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      window.open('https://vibemostwanted.xyz/fid/how-to-complete', '_blank');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-vintage-charcoal to-vintage-deep-black flex items-center justify-center p-4">
      <div className="max-w-sm w-full text-center">
        ...
        <p className="text-vintage-ice/80 text-sm mb-8">
          This page is now on <span className="text-vintage-gold font-bold">Vibe Most Wanted</span>.
        </p>
        <button
          onClick={handleOpen}
        ...
        <p className="text-vintage-ice/40 text-xs mt-6">vibemostwanted.xyz/fid</p>
      </div>
    </div>
  );
}
