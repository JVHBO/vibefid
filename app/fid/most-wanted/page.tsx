...
export default function MostWantedMigrated() {
  useEffect(() => {
    const init = async () => {
      try {
  ...
  const handleOpen = async () => {
    ...
    try {
      ...
      window.open('https://vibemostwanted.xyz/fid/most-wanted', '_blank');
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-vintage-charcoal to-vintage-deep-black flex items-center justify-center p-4">
      <div className="max-w-sm w-full text-center">
        ...
        <p className="text-vintage-ice/80 text-sm mb-2">
          Most Wanted ranking is now on <span className="text-vintage-gold font-bold">Vibe Most Wanted</span>.
        </p>
        ...
        <p className="text-vintage-ice/40 text-xs mt-6">vibemostwanted.xyz/fid/most-wanted</p>
      </div>
    </div>
  );
}
